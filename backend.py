...
class CaiBackend:
    ...
    def get_steam_path(self) -> Path | None:
        try:
            ...
            self.log.error(
                f'获取Steam路径失败: {self.stack_error(e)}。请检查Steam是否正确安装，或在config.json中设置Custom_Steam_Path。')
            return None
    ...
    async def checkcn(self) -> bool:
        try:
            ...
            if is_cn:
                self.log.info(f"检测到区域为中国大陆 ({body['country']})。将使用国内镜像。")
            else:
                self.log.info(f"检测到区域为非中国大陆 ({body['country']})。将直接使用GitHub。")
            return is_cn
        except Exception:
    ...
    async def _process_zip_manifest_generic(self, app_id: str, download_url: str, source_name: str) -> bool:
        ...
        try:
            ...
            if not is_auto_update_mode:  # 传统模式: 复制清单
                ...
                for f in manifest_files: shutil.copy2(f, steam_depot_path / f.name); self.log.info(
                    f'已复制清单: {f.name}')
    ...
    async def process_printedwaste_manifest(self, app_id: str) -> bool:
        return await self._process_zip_manifest_generic(app_id, f'https://api.printedwaste.com/gfk/download/{app_id}',
                                                        "SWA V2 (printedwaste)")
    ...
    async def process_assiw_manifest(self, app_id: str) -> bool:
        return await self._process_zip_manifest_generic(app_id, f'https://assiw.cngames.site/qindan/{app_id}.zip',
                                                        "CNGS (assiw)")

    async def process_steamdatabase_manifest(self, app_id: str) -> bool:
        return await self._process_zip_manifest_generic(app_id,
    ...
    async def process_github_manifest(self, app_id: str, repo: str) -> bool:
        ...
        try:
            if files_to_download:
                ...
                downloaded_files = {item['path']: content for item, content in
                                    zip(files_to_download, downloaded_contents)}
            else:
                downloaded_files = {}
        except Exception as e:
